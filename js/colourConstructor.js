class Colour {
    constructor(input) {
        if (typeof(input) === 'string') {
            ...
            this.luminance = input;
            this.rgb = this.generateRgbFromLuminance(input);
    ...
    rgbToLuminance(rgb) {
        return 0.2126 * this.srgbToLinear(rgb.r/255) + 0.7152 * this.srgbToLinear(rgb.g/255) + 0.0722 * this.srgbToLinear(rgb.b/255);
    }
    ...
    generateRgbFromLuminance(targetLuminance) {
        ...
        if (fixedChannels.includes('r') && fixedChannels.includes('g')) {
            ...
            g = this.clamp(this.linearToSrgb(this.randomInRange(isDark ? 0 : (targetLuminance - 0.2126 * this.srgbToLinear(r)) / 0.7152, isDark ? (targetLuminance - 0.2126 * this.srgbToLinear(r)) / 0.7152 : 1)), 0, 1);
            b = this.clamp(this.linearToSrgb((targetLuminance - 0.2126 * this.srgbToLinear(r) - 0.7152 * this.srgbToLinear(g)) / 0.0722), 0, 1);
            console.log("b = " + b);
        } else if (fixedChannels.includes('r') && fixedChannels.includes('b')) {
            console.log("g will be calculated");
            r = this.randomInRange(0, 1);
            b = this.clamp(this.linearToSrgb(this.randomInRange(isDark ? 0 : (targetLuminance - 0.2126 * this.srgbToLinear(r)) / 0.0722 / 0.7152, isDark ? (targetLuminance - 0.2126 * this.srgbToLinear(r)) / 0.0722 : 1)), 0, 1);
            g = this.clamp(this.linearToSrgb((targetLuminance - 0.2126 * this.srgbToLinear(r) - 0.0722 * this.srgbToLinear(b)) / 0.7152), 0, 1);
            console.log("g = " + g);
        } else {
            console.log("r will be calculated");
            g = this.randomInRange(0, 1);
            b = this.clamp(this.linearToSrgb(this.randomInRange(isDark ? 0 : targetLuminance - 0.7152 * this.srgbToLinear(g) / 0.0722, isDark ? targetLuminance - 0.7152 * this.srgbToLinear(g) / 0.0722 : 1)), 0, 1);
            r = this.clamp(this.linearToSrgb((targetLuminance - 0.7152 * this.srgbToLinear(g) - 0.0722 * this.srgbToLinear(b)) / 0.2126), 0, 1)
            console.log("r = " + r);
        }
